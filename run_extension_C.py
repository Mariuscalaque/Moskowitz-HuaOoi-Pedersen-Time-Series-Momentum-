"""Extension C — Dépendance de queue TSMOM–Marché par classe d'actifs.

Idée : les Extensions A et B ont analysé la dépendance de queue de TSMOM
...
Procédure (identique à Extension A, répétée par classe) :
...
Sorties :
  ext_C_copula_by_class.csv          — tableau récapitulatif par classe
...
from src.config import (
    FIGURES_DIR, TABLES_DIR, LOOKBACK_MONTHS, PAPER_START, PAPER_END,
    asset_class_of,
)
...
# ------------------------------------------------------------------ #
print("=== Extension C — Dépendance de queue TSMOM–Marché par classe d'actifs ===\n")
...
out_csv = TABLES_DIR / "ext_C_copula_by_class.csv"
...
fig2.suptitle(
    "Extension C — Résumé comparatif : λ_L, λ_U, asymétrie, famille",
    fontsize=11,
)
