"""Extension C — Stabilité temporelle de la dépendance TSMOM–Marché (Rolling Copula).

Idée : l'Extension B a estimé la copule SUR TOUT L'ÉCHANTILLON d'un coup.
...
CORRECTIONS (revue méthodologique) :
  - RF chargé via le loader robuste du projet (src.factors.fetch_ff_factors),
    plus de open() en dur -> identique à l'Extension B, zéro problème de chemin.
...
Sorties :
  ext_C_rolling_copula.csv        — série roulante complète
  ext_C_regime_statistics.csv     — stats par régime (Gaussian vs tail-dep.)
...
_TSMOM_CSV = PathLib(_os.environ.get("TSMOM_SERIES_CSV", "outputs/tables/diversified_tsmom_series.csv"))
...
# =========================================================
# (1) Charger les données — strictement identique à Ext B
# =========================================================
...
}]).to_csv(TABLES_DIR / "ext_C_panic_copula.csv", index=False)
...
fig.suptitle(
    f"Extension C — Stabilité temporelle de la dépendance TSMOM–Marché\n"
...
fig2.suptitle("Extension C — Résumé des régimes de dépendance", fontsize=11)
...
# =========================================================
TABLES_DIR.mkdir(parents=True, exist_ok=True)
rolling.reset_index().to_csv(TABLES_DIR / "ext_C_rolling_copula.csv", index=False)
...
regime_stats_flat.to_csv(TABLES_DIR / "ext_C_regime_statistics.csv")

# =========================================================
# (7) Résumé synthétique
# =========================================================
print("\n" + "=" * 55)
print("RÉSUMÉ EXTENSION C — Rolling Copula TSMOM–Marché")
print("=" * 55)
...
print("\nFichiers produits :")
print("  ext_C_rolling_copula.csv    — série roulante complète")
print("  ext_C_regime_statistics.csv — stats par régime (aplaties)")
